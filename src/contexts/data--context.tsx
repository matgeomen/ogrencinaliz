...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initialData = savedData ? JSON.parse(savedData) : mockStudentData.map(d => ({...d, id: `${d.student_no}-${d.exam_name}`}));
  ...
  const addStudentData = useCallback(async (newData: StudentExamResult[]) => {
    ...
    if ((storagePreference === 'cloud' || storagePreference === 'both') && firestore) {
        ...
        newData.forEach(item => {
            ...
            const docId = `${item.student_no}-${item.exam_name}`.replace(/[\/.]/g, '-');
